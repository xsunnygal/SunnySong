...
use commands::{
    add_music_directory, add_song_to_playlist, background_app, begin_jellyfin_quick_connect,
    clear_youtube_auth, connect_jellyfin_password, create_listening_profile, create_playlist,
    delete_listening_profile, download_song, finish_jellyfin_quick_connect,
    get_active_listening_profile, get_app_status, get_artist_page, get_collection_songs,
    get_developer_diagnostics, get_discovery_enabled, get_jellyfin_servers,
    get_library_album_tracks, get_library_albums, get_library_folders, get_library_tracks,
    get_liked_song_ids, get_listening_profiles, get_local_artists, get_music_directories,
    get_playback_state, get_playlist_tracks, get_playlists, get_quick_picks, get_recent_songs,
    get_song_lyrics, get_youtube_auth_status, hydrate_playback_queue,
    pick_android_download_directory, play_next, play_previous, play_queue_item, prepare_playback,
    refill_playback_queue, refresh_jellyfin_libraries, refresh_playback_source,
    remove_jellyfin_server, remove_music_directory, rename_listening_profile, report_playback,
    rescan_music_library, search_catalog, search_local_music, search_music,
    ...
    sync_media_session, validate_jellyfin_server, warm_playback_source,
};
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let database_path = data_dir.join("solmusic.sqlite3");
            ...
            app.manage(LocalArtworkStore::new(data_dir.join("artwork-cache"))?);
            app.manage(MediaProxy::start(data_dir.join("media-cache"))?);

...
<script lang="ts">
  ...
  import { getQuickPicks, getRecentSongs, type QuickPickOptions, type Recommendation, type Song } from "$lib/api/backend";
  ...
  let recentCursor = $state<number | null>(null);
  ...
  async function loadRecent(reset = false) {
    ...
    recentError = "";
    try {
      const page = await getRecentSongs(10, reset ? null : recentCursor);
      recent = reset ? page.items : [...recent, ...page.items.filter((song) => !recent.some((item) => item.id === song.id))];
  ...
  function activateProfile(profileId: string, revision: number) {
    ...
    const cache = profileQuickPickCaches.get(profileId);
    picks = cache?.picks ?? [];
  ...
  function recentScrolled(event: Event) {
    const element = event.currentTarget as HTMLElement;
    if (recentHasMore && !recentLoading && element.scrollLeft + element.clientWidth >= element.scrollWidth - 160) void loadRecent();
  }

  async function refreshHome() {
    const previousPickIds = picks.map((pick) => pick.song.id);
    await Promise.all([loadRecent(true), loadPicks(true, QUICK_PICK_PAGE_SIZE, previousPickIds)]);
    void prefetchFollowingPage();
  }
...
<div class="home-page" role="region" aria-label="Home content" ontouchstart={homeTouchStart} ontouchmove={homeTouchMove} ontouchend={homeTouchEnd} style={`--pull-distance: ${pullDistance}px`}>
  ...
  <section class="home-section" aria-labelledby="recent-title">
    <div class="section-heading">
      ...
      {#if recentHasMore}<button class="text-button" type="button" onclick={() => loadRecent()} disabled={recentLoading}>More <span aria-hidden="true">→</span></button>{/if}
    </div>
    {#if recent.length}
      <div class="recent-strip" onscroll={recentScrolled}>
        {#each recent as song (song.id)}<RecentCard {song} />{/each}
        {#if recentLoading}<div class="recent-loading" aria-label="Loading more recently played tracks"></div>{/if}
      </div>
    ...
    {:else if recentError}
      <p class="inline-message">Could not load recent plays. <button class="text-button" onclick={() => loadRecent(true)}>Retry</button></p>
    {/if}
  </section>
  ...
  <section class="home-section quick-picks" aria-labelledby="quick-picks-title">
      ...
      <div class="quick-pick-heading-actions">
        <div class="quick-pick-options">
          ...
          {#if pickOptionsOpen}<div class="quick-pick-options-menu">
            <label><input type="checkbox" checked={pickOptions.diverse} onchange={(event) => setPickOption("diverse", event.currentTarget.checked)} /><span><strong>More diverse</strong><small>Limit repeated artists</small></span></label>
            <label><input type="checkbox" checked={pickOptions.newSongs} onchange={(event) => setPickOption("newSongs", event.currentTarget.checked)} /><span><strong>New songs</strong><small>Favor music you have not heard</small></span></label>
            <label><input type="checkbox" checked={pickOptions.rediscover} onchange={(event) => setPickOption("rediscover", event.currentTarget.checked)} /><span><strong>Rediscover</strong><small>Bring back older favorites</small></span></label>
          </div>{/if}
        </div>
    ...
    {#if visiblePicks.length || pickPage > 0 || picksLoading || picksPrefetching}
      {#key pickPage}
        <div class="quick-page" role="group" aria-label={`Quick Picks page ${pickPage + 1}. Swipe horizontally to change page.`} ontouchstart={quickTouchStart} ontouchend={quickTouchEnd}>
          {#each Array(QUICK_PICK_PAGE_SIZE) as _, index}
            ...
            {#if pick}
              <div class="quick-pick-row" role="group" aria-label={`Quick Pick ${pickPage * QUICK_PICK_PAGE_SIZE + index + 1}`} onfocusin={pickReached} onpointerenter={pickReached}>
                <SongRow song={pick.song} detail={pick.song.id.startsWith("local:") || pick.song.id.startsWith("jellyfin:") ? "LIBRARY" : "YT"} />
              </div>
            {:else}<div class="row-skeleton skeleton" aria-label="Loading recommendation"></div>{/if}
          {/each}
        </div>
      {/key}
    ...
    {:else}
      <div class="empty-home">
        <p>{picksError || "Start listening and recommendations will appear here."}</p>
        <a class="primary-action" href="#/search">Search music</a>
      </div>
    {/if}
  </section>
</div>
